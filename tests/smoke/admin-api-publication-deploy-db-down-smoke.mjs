...
async function main() {
  ...
  try {
    ...
    assertCondition(login.response.status === 200, 'admin_api_publication_deploy_db_down_login_status');

    let cookie = mergeCookie('', login.response.headers.get('set-cookie'));
    assertCondition(cookie !== '', 'admin_api_publication_deploy_db_down_login_cookie');
    ...
    assertCondition(list.response.status === 200, 'admin_api_publication_deploy_db_down_list_status');
    ...
    if (!Number.isFinite(publicationId) || publicationId <= 0) {
      ...
      assertCondition(programmes.response.status === 200, 'admin_api_publication_deploy_db_down_programmes_status');

      const programmeId = Number(programmes.json?.items?.[0]?.id ?? 0);
      assertCondition(
        Number.isFinite(programmeId) && programmeId > 0,
        'admin_api_publication_deploy_db_down_programme_id_invalid'
      );
      ...
      assertCondition(created.response.status === 201, 'admin_api_publication_deploy_db_down_create_status');

      publicationId = Number(created.json?.item?.id ?? 0);
      assertCondition(
        Number.isFinite(publicationId) && publicationId > 0,
        'admin_api_publication_deploy_db_down_created_publication_id_invalid'
      );
    }
    ...
    try {
      ...
      const deploy = await fetchJson(`${apiBaseUrl}/api/publications/${publicationId}/deploy`, {
        method: 'POST',
        headers: { cookie, 'content-type': 'application/json' },
        body: JSON.stringify({ mode: 'local' })
      });

      assertCondition(deploy.response.status === 500, 'admin_api_publication_deploy_db_down_status');
      assertCondition(deploy.json?.ok === false, 'admin_api_publication_deploy_db_down_ok_false');
      assertCondition(
        String(deploy.json?.error || '') === 'publication_deploy_failed',
        'admin_api_publication_deploy_db_down_error'
      );
      assertCondition(String(deploy.json?.message || '') !== '', 'admin_api_publication_deploy_db_down_message');

      console.log('[smoke] admin-api publication deploy db-down contract ok');
      console.log(`[smoke] endpoint=/api/publications/${publicationId}/deploy with invalid mysql port`);
      hasFailure = false;
    } finally {
      phpDown.server.kill();
      await sleep(150);
